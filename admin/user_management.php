...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['permission'])) {
    ...
    $sql = "UPDATE user_details SET permission=? WHERE account_id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("si", $permission, $account_id);
    $stmt->execute();
}

$users = $conn->query("SELECT account_id, username, account_email, permission FROM user_details");
?>
                    ...
                    <td>
                        <form method="POST">
                            ...
                            <select name="permission">
                                <option value="allow" <?php if($user['permission'] == 'allow') echo 'selected'; ?>>Allow</option>
                                <option value="pending" <?php if($user['permission'] == 'pending') echo 'selected'; ?>>Pending</option>
                                <option value="reject" <?php if($user['permission'] == 'reject') echo 'selected'; ?>>Reject</option>
                            </select>
